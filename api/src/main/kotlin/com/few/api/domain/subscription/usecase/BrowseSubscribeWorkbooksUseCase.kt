...
class BrowseSubscribeWorkbooksUseCase(
    ...
    fun execute(useCaseIn: BrowseSubscribeWorkbooksUseCaseIn): BrowseSubscribeWorkbooksUseCaseOut {
        ...
        val subscriptionWorkbookCountRecords =
            subscriptionDao.countAllWorkbookSubscription(
                CountAllWorkbooksSubscriptionQuery(subscriptionWorkbookIds),
            )
        val subscriptionWorkbookSendStatusRecords =
            subscriptionDao
        ...
        val workbookDetails =
            workbookStatusRecords.map {
                SubscribeWorkbookDetail(
                    ...
                    subscription =
                        subscriptionWorkbookSendStatusRecords[it.workbookId]?.let { record ->
                            Subscription(
        ...
        return when (strategy) {
            SUBSCRIBE_WORKBOOK_STRATEGY.MAIN_CARD -> {
                val workbookSubscriptionCurrentArticleIdRecords =
                    workbookStatusRecords.associate { record ->
                        val articleId =
                            subscriptionArticleService.readArticleIdByWorkbookIdAndDay(
            ...
            SUBSCRIBE_WORKBOOK_STRATEGY.MY_PAGE -> {
                val workbookTitleRecords =
                    subscriptionWorkbookService.readAllWorkbookTitle(
                                ...
                                workbookInfo =
                                    objectMapper.writeValueAsString(
                                        WorkbookInfo(
                                            id = it.workbookId,
                                            title =
                                                workbookTitleRecords[it.workbookId]
