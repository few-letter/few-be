    ...
    BehaviorSpec({
        ...
        Given("정상적인 그룹 스케줄링 실행 시") {
            When("모든 카테고리에서 성공적으로 GroupGen을 생성하는 경우") {
                Then("성공 이벤트가 발행된다") {
                    val gens = makeGens(1L..5L)
                    val genDetails = gens.map { gen -> GenDetail("Headline ${gen.id}", "keyword1, keyword2") }
                    ...
                    every { genService.findAllByCreatedAtTodayAndCategoryAndRegion(any(), Region.GLOBAL) } returns gens
                    coEvery { keywordExtractor.extractKeywordsFromGens(gens) } returns genDetails
                    every { genGrouper.performGrouping(genDetails, any()) } returns group
                    every { genGrouper.validateGroupSize(group) } returns group
                    every { groupContentGenerator.generateGroupContent(any(), gens, group, Region.GLOBAL) } returns groupGen
        ...
        Given("GroupGen 생성 조건을 만족하지 못하는 경우") {
            ...
            When("그룹화에 실패하는 경우") {
                Then("BadRequestException이 발생한다") {
                    val gens = makeGens(1L..5L)
                    val genDetails = gens.map { gen -> GenDetail("Headline ${gen.id}", "keyword1") }

                    every { genService.findAllByCreatedAtTodayAndCategoryAndRegion(Category.TECHNOLOGY, Region.GLOBAL) } returns gens
                    coEvery { keywordExtractor.extractKeywordsFromGens(gens) } returns genDetails
                    every { genGrouper.performGrouping(genDetails, Category.TECHNOLOGY) } returns Group(listOf(0, 1))
                    every { genGrouper.validateGroupSize(any()) } returns null
        ...
        Given("GroupGen 생성 성공 시 메트릭 기록") {
            When("정상적으로 GroupGen이 생성되면") {
                Then("성공 메트릭이 기록된다") {
                    ...
                    every { genService.findAllByCreatedAtTodayAndCategoryAndRegion(Category.TECHNOLOGY, Region.GLOBAL) } returns gens
                    coEvery { keywordExtractor.extractKeywordsFromGens(gens) } returns genDetails
                    every { genGrouper.performGrouping(genDetails, Category.TECHNOLOGY) } returns group
                    every { genGrouper.validateGroupSize(group) } returns group
                    every { groupContentGenerator.generateGroupContent(Category.TECHNOLOGY, gens, group, Region.GLOBAL) } returns groupGen
