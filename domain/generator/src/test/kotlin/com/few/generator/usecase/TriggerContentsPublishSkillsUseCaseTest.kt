 ...
 * - 마지막 Given 은 UseCase 코드가 참조하는 실제 `.claude/scripts/publish-contents-common.sh` 파일을 대상으로 한다.
        ...
        if (zshAvailable) {
            Given("정상 종료(exit 0)하는 발행 스크립트가 있을 때") {
                ...
                writeScript(
                    ...
                    """
                    echo "running publish-contents-common"
            ...
            Given("타임아웃 시간(0분) 안에 끝나지 않는 스크립트가 있을 때") {
                writeScript("publish-contents-common.sh", "sleep 5")
            ...
            Given("모든 ContentsType 이 공통 스크립트를 사용할 때") {
                val marker = File(scriptsDir, "m-common.txt")
                writeScript("publish-contents-common.sh", "touch '${marker.absolutePath}'")

                ContentsType.entries.forEach { contentsType ->
                    When("$contentsType 으로 execute 하면") {
                        marker.delete()
                        newUseCase().execute(contentsType)

                        Then("publish-contents-common.sh 가 실행된다") {
                            marker.exists() shouldBe true
                        }
                    }
                }
            ...
            Given("UseCase 코드가 참조하는 실제 .claude/scripts/publish-contents-common.sh 파일") {
                var repoRoot: File? = File(System.getProperty("user.dir")).absoluteFile
                while (repoRoot != null && !File(repoRoot, ".claude/scripts/publish-contents-common.sh").isFile) {
                    repoRoot = repoRoot.parentFile
                }
                val realScript = repoRoot?.let { File(it, ".claude/scripts/publish-contents-common.sh") }
                ...
                When("RUN_REAL_PUBLISH_SCRIPT=true 인 경우 실제 UseCase.execute 로 실행하면") {
                    ...
                    Then("파일 미존재/타임아웃 없이 프로세스가 정상적으로 끝까지 실행된다").config(enabled = runReal) {
                        val realUseCase =
                            TriggerContentsPublishSkillsUseCase(
                                publishScriptPath = realScript!!.absolutePath,
