...
class GlobalGroupGenSchedulingUseCase(
    ...
    fun execute() {
        if (!isRunning.compareAndSet(false, true)) {
            throw BadRequestException("Global group scheduling is already running. Please try again later.")
        }
    ...
    private fun doExecute() {
        ...
        runCatching {
            ...
            isSuccess = false
            log.error(ex) { "Global 그룹 스케줄링 중 오류 발생" }
            exception = ex
        }.also {
            log.info {
                buildString {
                    appendLine("✅ Global Group Scheduling Result")
            ...
            if (!isSuccess) {
                throw BadRequestException("Global 그룹 스케줄링에 실패 : ${exception?.cause?.message}")
            }
        }
    }

    fun createGroupGens(): List<GroupGen> {
        log.info { "GLOBAL 전체 카테고리 그룹 생성 시작" }
        ...
        categories.forEach { category ->
            try {
                ...
                log.info { "GLOBAL 카테고리 ${category.title} 그룹 생성 완료" }
            } catch (e: Exception) {
                log.error(e) { "GLOBAL 카테고리 ${category.title} 그룹 생성 실패하여 Skip" }
            }
        }

        log.info { "GLOBAL 전체 카테고리 그룹 생성 완료: ${results.size}개" }
        return results
    }

    fun createGroupGen(category: Category): GroupGen {
        log.info { "GLOBAL 그룹 생성 시작: category=${category.title}" }
        ...
        val totalProcessingTime =
            measureTimeMillis {
                try {
                    ...
                    log.error(e) { "GLOBAL 그룹 생성 중 오류 발생: category=${category.title}, 전체 Gen 수: $totalGens" }
                    processingError = e
                }
            }
        ...
        if (finalResult == null || (finalResult.headline.isEmpty() && finalResult.summary.isEmpty())) {
            log.warn {
                "GLOBAL 그룹 생성 실패 또는 빈 결과: category=${category.title}, headline=${finalResult?.headline?.isNotEmpty()}, summary=${finalResult?.summary?.isNotEmpty()}"
            }
        ...
        return result ?: throw BadRequestException("GLOBAL Group Gen 생성 실패 - Cause: Unknown (카테고리: ${category.title})")
    }
    ...
    private suspend fun createGroupGenInternalWithMetrics(category: Category): GroupGenProcessingResult {
        ...
        if (gens.isEmpty()) {
            throw BadRequestException("GLOBAL Group Gen 생성 실패 - Cause: 카테고리 ${category.title}에 대한 Gen이 없습니다.")
        }

        if (gens.size < groupingProperties.minGroupSize) {
            throw BadRequestException(
                "GLOBAL Group Gen 생성 실패 - Cause: 카테고리 ${category.title}의 Gen 개수(${gens.size})가 최소 그룹 크기(${groupingProperties.minGroupSize})보다 작습니다.",
            )
        }

        log.info { "GLOBAL 카테고리 ${category.title}에서 ${gens.size}개 Gen 발견, 키워드 추출 시작" }
        ...
        if (validatedGroup == null) {
            throw BadRequestException("GLOBAL Group Gen 생성 실패 - Cause: 카테고리 ${category.title} Gen Grouping 실패")
        }
