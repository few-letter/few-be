...
@Component
class NewsCrawlerAppleInsider(
    private val generatorRestTemplate: RestTemplate,
) : BaseNewsCrawler(generatorRestTemplate) {
    companion object {
        private const val PRESS_NAME = "AppleInsider"
        private const val BASE_URL = "https://appleinsider.com"
    ...
    override fun getNewsUrls(
        ...
        logger.info { "AppleInsider 뉴스 링크를 수집합니다." }
        val allArticles = mutableListOf<String>()

        for (page in 1..MAX_PAGES) {
            val url = "$BASE_URL/news/page/$page"
    ...
    private fun fetchPage(url: String): List<String> {
        ...
        if (soup != null) {
            val mainArticles: Elements = soup.select("article.main-art.river")
            for (article in mainArticles) {
                val contentDiv = article.selectFirst("div.col-sm-8") ?: continue
                val titleElem = contentDiv.selectFirst("h2.blueHover a")
