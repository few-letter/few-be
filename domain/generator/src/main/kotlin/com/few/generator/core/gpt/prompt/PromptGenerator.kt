...
class PromptGenerator(
    ...
    fun toKoreanHighlightText(summary: String): Prompt {
        ...
        val userPrompt =
            ...
            2. 하이라이트는 한 문장 또는 핵심 단어나 구문으로 추출하세요. 길 경우(대략 13자 이상) 원문 일부만 발췌하세요.
            3. 반드시 아래 Input의 원문과 철자·띄어쓰기·숫자·기호까지 100% 일치해야 합니다.
            4. 하이라이트 텍스트를 2~3개 추출해야 하며, 결과들을 'highlightTexts' 필드에 리스트로 담아 응답하세요.
            5. 강조: 출력은 아래 Input에서 그대로 복사한 문자열이어야 합니다. 문장 전체가 아니어도 되며, 단어 또는 일부 구문이어도 됩니다. 단, 원문을 재구성하지 말고 그대로 인용하시오.
    ...
    fun toInstagramHashtags(
        ...
        val userPrompt =
            ...
            7. 각 키워드에 공백(띄어쓰기)이 포함되지 않도록 반드시 붙여쓰세요. 예: "서울 맛집" → "서울맛집", "삼성 전자" → "삼성전자"
    ...
    fun toKoreanShortHeadline(title: String): Prompt {
        ...
        val userPrompt =
            ...
            4. 문장형 종결어미(~입니다, ~했다 등)를 사용하지 말고, '명사로 끝나는 구문'으로 작성하세요.(예: '애플의 신제품 발표', '삼성 주가 급등' 등)
            5. 전체 내용을 한국어로 번역하되, 주요 고유명사는 영어로 유지해야 합니다.
    ...
    fun toKoreanShortSummary(summary: String): Prompt {
        ...
        val userPrompt =
            ...
            4. 전체 내용을 한국어로 번역하되, 주요 고유명사는 영어로 유지해야 합니다.
